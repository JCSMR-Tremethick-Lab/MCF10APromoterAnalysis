...
samples <- list.files(kallistoDir, pattern = 'D8')

s2c <- data.table(sample = samples, condition = c(rep('WT', 3), rep('shZ', 3)))
...
so <- sleuth::sleuth_wt(so, 'conditionshZ') 

rt <- sleuth::sleuth_results(so, 'conditionshZ', show_all = F)
setDT(rt)
