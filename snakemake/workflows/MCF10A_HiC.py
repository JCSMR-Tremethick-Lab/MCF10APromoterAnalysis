...
rule create_domains_file:
    ...
    shell:
        """"
            cat {input} | sort -k1,1 -k2,2n - | {params.bedtools_location}/mergeBed -i - | awk '{print "chr"$1 "\t" $2 "\t" $3 "\tdomain"}' - | {params.bedtools_location}/bedtools complement -i {input} -g {params.genomeSizeFile} | sort -k1,1 -k2,2n - | awk '{if($4=="domain") print $0; else print $1 "\t" $2 "\t" $3 "\tgap"}' > {output}
        """"
